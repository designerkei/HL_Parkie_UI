...
test('button state contrast and focus ring visibility are enforced', async ({ page }) => {
  ...
  /* The primary ramp uses dark brand ink on three bright brand surfaces, so
     every documented state must clear the contrast floor. */
  const failing = page.locator('tr:has(.pk-token-verdict.is-fail)');
  await expect(failing, 'all documented button states must pass').toHaveCount(0);
