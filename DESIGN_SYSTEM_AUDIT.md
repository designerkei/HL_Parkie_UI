...
포커스 표본을 고치면서 아이콘 카탈로그를 Material 규격에 대조했다. 고친 것은 포커스
하나뿐이고, 아래 셋은 **확인만 하고 남겨두었다.** 셋 다 전달 표본을 건드리는 결정이라
전달자와 확인이 필요하다.
...
2. **`Refresh`가 라이브 영역을 넘는다.** 30행의 도형 경계를 실측한 결과 세로 21.5dp로,
   20dp 라이브 영역을 초과하고 24 그리드 안에서 위아래 여백이 1.25dp뿐이다. 유일한 초과다
...
**측정 방법** — 페이지를 렌더한 뒤 각 행의 첫 SVG에서 모든 도형의 `getBBox()`를 합집합해
경계를 구했다. 토큰이나 소스 문자열이 아니라 브라우저가 실제로 그린 결과다.
